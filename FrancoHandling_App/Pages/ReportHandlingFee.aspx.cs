...
{
    public partial class ReportHandlingFee : System.Web.UI.Page
    {
        ...
        protected void cBackReport_Callback(object sender, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            DataTable dTabel = dataReport.GetDataReportHandlingFee(datePeriodeStart.Date, datePeriodeEnd.Date, Convert.ToInt32(cmbTBBM.SelectedItem.Value), cmbForce.SelectedItem.Value.ToString(), cmbUnity.SelectedItem.Value.ToString());
            string region = dataReport.GetRegion(cmbTBBM.SelectedItem.Value.ToString());
            Report.xReportHandlingFee report = new FrancoHandling_App.Report.xReportHandlingFee();                     

            report.xrLabel_Title1.Text = "REKAPITULASI PENYALURAN FRANCO BBM " + cmbForce.SelectedItem.Value.ToString().ToUpper() + " DI " + cmbTBBM.SelectedItem.Text.ToUpper() + " " + region;
